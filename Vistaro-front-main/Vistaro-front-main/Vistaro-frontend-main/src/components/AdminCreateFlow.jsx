...
export default function AdminEventCreateModal({ isOpen, onClose }) {
  ...
  const validateBasicInfo = () => {
    if (!eventType) {
      toast.warning( "Select event type (Movie/Sport/Event)" );
      return false;
    }
    ...
    if (!eventForm.startTime || !eventForm.endTime) {
      toast.warning("Start and end time are required" );
      return false;
    }
    if (new Date(eventForm.startTime) >= new Date(eventForm.endTime)) {
      toast.warning("Event start time must be before end time",);
      return false;
    }
    return true;
  };
  ...
  const validateSlots = () => {
    if (!slots.length) {
      toast.warning( "Add at least one slot" );
      return false;
    }
    for (const [idx, s] of slots.entries()) {
      if (!s.venueId) {
        toast.warning(`Select a venue for Slot #${idx + 1}`);
        return false;
      }
      if (!s.startTime || !s.endTime) {
        toast.warning(`Start & end time are required for Slot #${idx + 1}`);
        return false;
      }
      if (new Date(s.startTime) >= new Date(s.endTime)) {
        toast.warning(`Start time must be before end time for Slot #${idx + 1}`);
        return false;
      }
      if (!s.language.trim()) {
        toast.warning(`Language is required for Slot #${idx + 1}`);
        return false;
      }
      if (!s.basePrice || Number(s.basePrice) <= 0) {
        toast.warning(`Base price must be > 0 for Slot #${idx + 1}`,);
        return false;
      }
    }
    return true;
  ...
  const validateTypeDetails = () => {
    if (eventType === "MOVIE") {
      if (
        ...
        toast.warning("Director, genre and language are required for Movie");
        return false;
      }
      ...
      if (
        ...
        toast.warning( "Sport type, team 1 and team 2 are required");
        return false;
      }
    } else if (eventType === "EVENT") {
      if (!generalDetails.artist.trim() || !generalDetails.genre.trim()) {
        toast.warning("Artist and genre are required for Event");
        return false;
      }
    }
    return true;
    ...
    >
      ...
      <ModalContent bg="#020617" color="gray.100" borderRadius="2xl">
            ...
            <AccordionItem isDisabled={!eventType}>
              ...
              <AccordionPanel pt={4} pb={6}>
                <VStack align="stretch" spacing={3}>
                  ...
                  <HStack justify="flex-end" pt={3}>
                    <Button variant="solid" onClick={() => goToStep(0)}>
                      Edit
                    </Button>
                    <Button
                      colorScheme="teal"
        ...
        <ModalFooter borderTop="1px solid" borderColor="gray.700">
          <Button variant="solid" onClick={onClose} mr={3}>
            Cancel
          </Button>
        </ModalFooter>
      </ModalContent>
