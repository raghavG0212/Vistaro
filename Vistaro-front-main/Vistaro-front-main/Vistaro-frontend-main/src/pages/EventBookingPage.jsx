...
export default function EventBookingPage() {
  ...
  const shouldAutoReleaseRef = useRef(true);
  ...
  const seatsByRow = useMemo(() => {
    ...
    // sort by numeric seat number (A1, A2, ... A10 — not A1, A10, A2)
    const getSeatNumberValue = (seat) => {
      const raw = seat.seatNumber || seat.seat_number || "";
    ...
    Object.keys(map).forEach((row) => {
      map[row].sort((a, b) => getSeatNumberValue(a) - getSeatNumberValue(b));
    });
  ...
  const ticketTotal = useMemo(() => {
    return selectedSeats.reduce((sum, seatId) => {
      const seat = seats.find((s) => (s.seatId || s.seat_id) === seatId);
      if (!seat) return sum;
  ...
  // ------------ AUTO RELEASE SEATS ON LEAVE (only if we haven't confirmed) -------------
  const releaseSeats = useCallback(async () => {
    const toUnlock = selectedSeatsRef.current;
    if (!toUnlock.length) return;
    if (!shouldAutoReleaseRef.current) return; // skip if we already confirmed
  ...
  const validatePaymentDetails = () => {
    ...
    if (paymentMode === "CARD") {
      ...
      if (!upiId.trim() || !upiId.includes("@")) {
        toast.error("Enter a valid UPI ID (e.g. your-name@bank).");
        return false;
      }
    }
  ...
  // ------------ CONFIRM PAYMENT HANDLER (LOCK + NAVIGATE) -------------
  const handleConfirmPayment = async () => {
    try {
      ...
      // we are proceeding to confirm → do NOT auto-release on unmount
      shouldAutoReleaseRef.current = false;

      // 1) LOCK SEATS in backend
      await lockSeats(selectedSeats);
      console.log("Seats locked from frontend:", selectedSeats);
      toast.success("Seats Locked");

      // 2) BUILD PAYLOAD & NAVIGATE TO CONFIRM PAGE
      ...
      const msg =
        ...
        err?.response?.data ||
        "Failed to lock seats. Try again.";
      toast.error(msg);
    }
  };
      ...
      <Box maxW="1200px" mx="auto">
        ...
        <Grid templateColumns={{ base: "1fr", lg: "2.2fr 1fr" }} gap={6}>
          ...
          <Box>
            <Accordion allowToggle defaultIndex={[0]}>
              ...
              >
                <AccordionButton px={4} py={4} _expanded={{ bg: "gray.700" }}>
                ...
                <AccordionPanel pb={4}>
                  ...
                  {filteredSlots.length === 0 ? (
                    <Text fontSize="sm" color="gray.400">
                      No upcoming shows available for this event.
                    </Text>
                  ) : (
              ...
              >
                ...
                <AccordionPanel pb={4}>
                  {!selectedSlot ? (
                    ...
                    <>
                      {/* Draw rows from back to front so SCREEN is near last row */}
                      <Stack spacing={2}>
                  ...
                  <Stack spacing={3}>
                    <Box>
                      ...
                      <Flex gap={2}>
                        <input
                          ...
                          placeholder="e.g. VIST50 / BANK10 / WEEKEND20"
                        />
                      </Flex>
                    </Box>
                    ...
                    <Stack spacing={1} fontSize="sm">
                      ...
                      <Text fontSize="xs" color="gray.500">
                        Final amount may change slightly depending on offer
                  ...
                  <Stack spacing={4}>
                    ...
                    {paymentMode === "CARD" && (
                      <Stack spacing={2} fontSize="sm">
                        <Text color="gray.400">
                          Enter your card details (demo validation only):
                        </Text>
                        <input
                          style={{
                            background: "#020617",
                    ...
                    {paymentMode === "UPI" && (
                      <Stack spacing={2} fontSize="sm">
                        <Text color="gray.400">
                          Enter any dummy UPI ID (validated for format):
                        </Text>
                        <input
                          style={{
                            background: "#020617",
        ...
        <Modal isOpen={isConfirmOpen} onClose={closeConfirm} isCentered>
          ...
          <ModalContent bg="gray.800" color="gray.100">
            ...
            <ModalCloseButton />
            <ModalPanelBody event={event} estimatedPayable={estimatedPayable} />
            <ModalFooterSection
              onCancel={() => {
                closeConfirm();
                // user backed out → allow auto release if they leave
                shouldAutoReleaseRef.current = true;
                // we hadn't locked yet, but just in case:
                releaseSeats();
              }}
...
// Small helper subcomponents for the modal body/footer
function ModalPanelBody({ event, estimatedPayable }) {
  return (
    <Box as="section" px={6} py={4}>
      <Text mb={2}>You are about to complete your booking for:</Text>
      ...
      <Text fontSize="xs" color="gray.500" mt={2}>
        Note: Card / UPI fields are validated on this screen, but payment is
        still a demo – booking will be created officially.
      </Text>
    </Box>
  );
}

function ModalFooterSection({ onCancel, onConfirm }) {
  return (
    <Box as="footer" px={6} py={4} display="flex" justifyContent="flex-end">
